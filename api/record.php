...
class RECORD extends API {
			...
			case 'GET':
				...
				if ($casestate = $this->casestate($content['context'], 'checkbox', ['onchange' => "api.record('patch', 'casestate', '" . $this->_requestedID . "', this.dataset.casestate, this.checked);"
					...
					if(!PERMISSION::permissionFor('recordscasestate')){
						unset($erp_case_number['attributes']['onkeydown']);
					...
					if (PERMISSION::permissionFor('erpcasedata') && ERPINTERFACE && ERPINTERFACE->_instatiated && method_exists(ERPINTERFACE, 'casepositions') && ERPINTERFACE->casepositions()){
						$general[] = [
							'type' => 'button',
							'attributes' => [
	...
	private function summarizeRecord($type = 'full', $export = false){
		...
		// shift pseudodocument to end
		if (isset($accumulatedcontent[$this->_lang->GET('record.altering_pseudodocument_name', [], $export)])){
