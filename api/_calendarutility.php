...
class CALENDARUTILITY {
	 ...
	 */
   	...
   	/**
	 * preset database connection, passed from main application CARO\API\SQLINTERFACE
	 */
	 ...
	 */
	private array $_holidays = [];
	private array $_easter_holidays = [];
	public array $_workdays = [];
	private array $_breaks = [];

	/**
	 * @param mixed $_sqlinterface
	 * @param array $date
	 */
	public function __construct($_sqlinterface, $date){
	...
	public function alert(){
		return $this->_sqlinterface->EXECUTE('calendar_alert');
	}
	 ...
	 *                |_|
	 * @param string $id eventually comma separated multiple ints
	 * @param mixed $close boolval false will clear
	 * @param mixed $alert null remains state, bool updates
	...
	public function complete($id = '0', $close = null, $alert = null){
		...
		$sqlQueryStack = [];
		$entries = $this->_sqlinterface->EXECUTE('calendar_get_by_id', [
			':id' => $id
		]);
		foreach ($entries as $entry){
			$sqlQueryStack = $this->_sqlinterface->PACK($sqlQueryStack, $this->_sqlinterface->PREPARE('calendar_complete',
				[
					':id' => $entry['id'],
	...
	public function delete($id = 0){
		if ($id !== null) return $this->_sqlinterface->EXECUTE('calendar_delete', [
			':ids' => [$id]
		]);
		$ids = [];
		...
		if ($ids) return $this->_sqlinterface->EXECUTE('calendar_delete', [
			':ids' => $ids
		]);
		return 0;
	}
	...
	public function getDay($date = ''){
		return $this->_sqlinterface->EXECUTE('calendar_get_day', [
			':date' => $date
		]);
	}
	 ...
	 * 
	 * @param mixed $earlier Y-m-d | null
	 * @param mixed $later Y-m-d | null
	 * 
	 * @return array sql result
	 */
	public function getWithinDateRange($earlier = '', $later = ''){
		return $this->_sqlinterface->EXECUTE('calendar_get_within_date_range', [
	...
	public function post($columns = []){
		if ($this->_sqlinterface->EXECUTE('calendar_post', $columns)) return $columns[':id'] ?? $this->_sqlinterface->_pdo->lastInsertId();
		return false;
	}
	 ...
	 * 
	 * @param string $format month|week
	 * @param array $types tasks|timesheet|worklists
	 * @param string $date yyyy-mm-dd
	 * 
	...
	public function timesheetSummary($users = [], $from_date = '', $to_date = ''){
		...
		$result_span_start = array_column($entries, 'span_start'); 
		array_multisort($result_span_start, SORT_ASC, $entries);
		...
		foreach ($users as $row => $user){
			...
			foreach (['weeklyhours', 'annualvacation'] as $setting){
				...
				$result_hours_vacation = array_column($hours_vacation, 'date'); 
				array_multisort($result_hours_vacation, SORT_ASC, $hours_vacation);
	...
	public function search($search = ''){
		return $this->_sqlinterface->EXECUTE('calendar_search', [
			':SEARCH' => $search
		],
		true);
	}
