...
class PDF{
	...
	private function init($content){
		...
		// set custom indentation width
		$this->_pdf->setListIndentWidth($this->_pdf->GetStringWidth('000'));
	}
	...
	public function orderPDF($content){
		...
		foreach ($content['content'] as $commission){
			...
			foreach ($commission as $key => $value){
				...
				// writeHTMLCell($w, $h, $x, $y, $html='', $border=0, $ln=0, $fill=false, $reseth=true, $align='', $autopadding=true)
				$valueLines = $this->_pdf->writeHTMLCell(145, 4, 60, $this->_pdf->GetY(), $this->_markdown_css . $_markdown->md2html($value, true, ["list", "emphasis", "larger", "br"]) . "<br />", 0, 1, 0, true, '', true);
