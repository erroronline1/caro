...
class SQLQUERY {
	...
	public const QUERIES = [
		...
		'consumables_put_product_productlist_erp_amendment' => [
			'mysql' => "UPDATE caro_consumables_products SET article_unit = IF(article_unit IS NULL, :article_unit, article_unit), article_ean = IF(article_ean IS NULL, :article_ean, article_ean), trading_good = IF(trading_good IS NULL, :trading_good, trading_good), incorporated = IF(incorporated IS NULL, :incorporated, incorporated), has_expiry_date = IF(has_expiry_date IS  NULL, :has_expiry_date, has_expiry_date), special_attention = IF(special_attention IS NULL, :special_attention, special_attention), stock_item = IF(stock_item IS NULL, :stock_item, stock_item), erp_id = IF(erp_id IS NULL, :erp_id, erp_id), last_order = IF(last_order IS NULL, :last_order, last_order) WHERE vendor_id = :vendor_id AND article_no = :article_no",
			'sqlsrv' => "UPDATE caro_consumables_products SET article_unit = (CASE WHEN article_unit IS NULL THEN :article_unit ELSE article_unit END), article_ean = (CASE WHEN article_ean IS NULL THEN :article_ean ELSE article_ean END), trading_good = (CASE WHEN trading_good IS NULL THEN :trading_good ELSE trading_good END), incorporated = (CASE WHEN incorporated IS NULL THEN :incorporated ELSE incorporated END), has_expiry_date = (CASE WHEN has_expiry_date IS NULL THEN :has_expiry_date ELSE has_expiry_date END), special_attention = (CASE WHEN special_attention IS NULL THEN :special_attention ELSE special_attention END), stock_item = (CASE WHEN stock_item IS NULL THEN :stock_item ELSE stock_item END), erp_id = (CASE WHEN erp_id IS NULL THEN :erp_id ELSE erp_id END), last_order = (CASE WHEN last_order IS NULL THEN CONVERT(SMALLDATETIME, :last_order, 120) ELSE last_order END) WHERE vendor_id = :vendor_id AND article_no = :article_no"
		],
