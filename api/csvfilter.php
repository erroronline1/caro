...
class CSVFILTER extends API {
		...
		switch ($_SERVER['REQUEST_METHOD']){
			case 'POST':
				...
				if (!$filter) $this->response([
					'response' => [
						'name' => false,
						'msg' => $this->_lang->GET('csvfilter.error_filter_not_found'),
						'type' => 'error'
					]]);
				$content = json_decode($filter['content'], true);

				$inputfile = isset($_FILES[$this->_lang->PROPERTY('csvfilter.use_filter_input_file')]) ? $_FILES[$this->_lang->PROPERTY('csvfilter.use_filter_input_file')]['tmp_name'] : null;
				if (!$inputfile) $this->response([
					'response' => [
						'name' => false,
						'msg' => $this->_lang->GET('csvfilter.use_filter_no_input_file'),
						'type' => 'error'
					]]);
				...
				foreach($content['filter'] as &$filtertype){
					if ($filtertype['apply'] === 'filter_by_comparison_file' && $filtertype['filesetting']['source'] !== 'SELF') {
						$comparefile = isset($_FILES[$this->_lang->PROPERTY('csvfilter.use_filter_compare_file')]) && isset($_FILES[$this->_lang->PROPERTY('csvfilter.use_filter_compare_file')]['tmp_name'][$comparefileindex]) ? $_FILES[$this->_lang->PROPERTY('csvfilter.use_filter_compare_file')]['tmp_name'][$comparefileindex] : null;
						if (!$comparefile) $this->response([
							'response' => [
								'name' => false,
								'msg' => $this->_lang->GET('csvfilter.use_filter_no_compare_file', [':name' => $filtertype['filesetting']['source']]),
								'type' => 'error'
							]]);
				...
				$datalist = new Listprocessor($content, [
					'processedMonth' => UTILITY::propertySet($this->_payload, $this->_lang->PROPERTY('csvfilter.use_filter_month')),
					'processedYear' => UTILITY::propertySet($this->_payload, $this->_lang->PROPERTY('csvfilter.use_filter_year'))
				]);
				...
				switch (strtolower(pathinfo($content['filesetting']['destination'])['extension'])){
					case 'csv':
						foreach($datalist->_list as $subsetname => $subset){
							...
							fclose($file);
							$downloadfiles[$this->_lang->GET('csvfilter.use_filter_download', [':file' => preg_replace('/.csv$/', (count($datalist->_list) > 1 ? '_' . $subsetname. '.csv' : '.csv'), $content['filesetting']['destination'])])] = [
					...
					case 'xlsx':
						...
						$downloadfiles[$this->_lang->GET('csvfilter.use_filter_download', [':file' => $content['filesetting']['destination']])] = [
							'href' => substr(UTILITY::directory('tmp'), 1) . '/' . pathinfo($tempFile)['basename'],
			...
			case 'GET':
				$filterdatalist = [];
				$options = ['...' . $this->_lang->GET('csvfilter.use_filter_select') => (!$this->_requestedID) ? ['value' => '0', 'selected' => true] : ['value' => '0']];
				$return = [];
				...
				if($this->_requestedID && $this->_requestedID !== 'false' && !$filter['name'] && $this->_requestedID !== '0') $return['response'] = ['msg' => $this->_lang->GET('csvfilter.error_template_not_found', [':name' => $this->_requestedID]), 'type' => 'error'];
					...
					'content' => [
						[
							[
								...
								'type' => 'select',
								'attributes' => [
									'name' => $this->_lang->GET('csvfilter.use_filter_select'),
								...
								'type' => 'search',
								'attributes' => [
									'name' => $this->_lang->GET('csvfilter.edit_filter'),
									'list' => 'filters',
				...
				if ($filter['id']){
					...
					$additionalform = [
						[
							'type' => 'file',
							'hint' => $this->_lang->GET('csvfilter.use_filter_input_file_hint', [':name' => $content['filesetting']['source']]),
							'attributes' => [
								'name' => $this->_lang->GET('csvfilter.use_filter_input_file'),
								'required' => true,
								'accept' => '.csv'
							]
						], [
							...
							'type' => 'number',
							'attributes' => [
								'name' => $this->_lang->GET('csvfilter.use_filter_month'),
							...
							'type' => 'number',
							'attributes' => [
								'name' => $this->_lang->GET('csvfilter.use_filter_year'),
					...
					foreach($content['filter'] as $filtertype){
						if ($filtertype['apply'] === 'filter_by_comparison_file' && $filtertype['filesetting']['source'] !== 'SELF') array_push($additionalform, [
							'type' => 'file',
							'hint' => $this->_lang->GET('csvfilter.use_filter_input_file_hint', [':name' => $filtertype['filesetting']['source']]),
							'attributes' => [
								'name' => $this->_lang->GET('csvfilter.use_filter_compare_file') . '[]',
								'required' => true,
								'accept' => '.csv'
							]
						]);
	...
	public function rule(){
		...
		switch ($_SERVER['REQUEST_METHOD']){
			case 'POST':
				$filter = [
					':name' => UTILITY::propertySet($this->_payload, $this->_lang->PROPERTY('csvfilter.edit_filter_name')),
					':author' => $_SESSION['user']['name'],
					':content' => UTILITY::propertySet($this->_payload, $this->_lang->PROPERTY('csvfilter.edit_filter_content')),
					':hidden' => UTILITY::propertySet($this->_payload, $this->_lang->PROPERTY('csvfilter.edit_filter_hidden')) === $this->_lang->PROPERTY('csvfilter.edit_filter_hidden_hidden')? 1 : 0,
				];

				if (!trim($filter[':name']) || !trim($filter[':content'])) $this->response([], 400);

				// ensure valid json for filters
				if ($filter[':content'] && !json_decode($filter[':content'], true))  $this->response(['response' => ['msg' => $this->_lang->GET('csvfilter.edit_filter_content_hint'), 'type' => 'error']]);
				...
				if ($exists && $exists['content'] === $filter[':content']) {
					if (SQLQUERY::EXECUTE($this->_pdo, 'csvfilter_put', [
						...
						'response' => [
							'name' => $filter[':name'],
							'msg' => $this->_lang->GET('csvfilter.edit_filter_saved', [':name' => $filter[':name']]),
							'type' => 'success'
						]]);	
				}

				foreach(CONFIG['forbidden']['names'] as $pattern){
					if (preg_match("/" . $pattern . "/m", $filter[':name'], $matches)) $this->response(['response' => ['msg' => $this->_lang->GET('csvfilter.error_forbidden_name', [':name' => $filter[':name']]), 'type' => 'error']]);
				}
				...
				if (SQLQUERY::EXECUTE($this->_pdo, 'csvfilter_post', [
					...
					'response' => [
						'name' => false,
						'msg' => $this->_lang->GET('csvfilter.edit_filter_not_saved'),
						'type' => 'error'
					]]);
				break;
			case 'GET':
				$filterdatalist = [];
				$options = ['...' . $this->_lang->GET('csvfilter.edit_filter_new') => (!$this->_requestedID) ? ['value' => '0', 'selected' => true] : ['value' => '0']];
				$alloptions = ['...' . $this->_lang->GET('csvfilter.edit_filter_new') => (!$this->_requestedID) ? ['value' => '0', 'selected' => true] : ['value' => '0']];
				$return = [];
				...
				if($this->_requestedID && $this->_requestedID !== 'false' && !$filter['name'] && $this->_requestedID !== '0') $return['response'] = ['msg' => $this->_lang->GET('csvfilter.error_filter_not_found', [':name' => $this->_requestedID]), 'type' => 'error'];
						...
						[
							[
								[
									...
									'type' => 'select',
									'attributes' => [
										'name' => $this->_lang->GET('csvfilter.edit_filter_select'),
								...
								[
									'type' => 'select',
									'attributes' => [
										'name' => $this->_lang->GET('csvfilter.edit_filter_all'),
										'onchange' => "api.csvfilter('get', 'rule', this.value)"
									],
									'content' => $alloptions
								]
							...
							[
								'type' => 'text',
								'attributes' => [
									'name' => $this->_lang->GET('csvfilter.edit_filter_name'),
									'value' => $filter['name'],
									'required' => true,
									'data-loss' => 'prevent'
								]
								...
								'type' => 'code',
								'hint' => $this->_lang->GET('csvfilter.edit_filter_content_hint'),
								'attributes' => [
									'name' => $this->_lang->GET('csvfilter.edit_filter_content'),
				...
				if ($filter['id']){
					$hidden = [
						'type' => 'radio',
						'attributes' => [
							'name' => $this->_lang->GET('csvfilter.edit_filter_hidden')
						],
						'content' => [
							$this->_lang->GET('csvfilter.edit_filter_hidden_visible') => ['checked' => true],
							$this->_lang->GET('csvfilter.edit_filter_hidden_hidden') => []
						],
						'hint' => $this->_lang->GET('csvfilter.edit_filter_hidden_hint')
					];
					if ($filter['hidden']) $hidden['content'][$this->_lang->GET('csvfilter.edit_filter_hidden_hidden')]['checked'] = true;
