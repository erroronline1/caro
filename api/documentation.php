   ...
   private $PDFLIBRARY = '../libraries/TCPDF/tcpdf_import.php';
	...
	public function identifier(){
		...
		switch ($_SERVER['REQUEST_METHOD']){
			case 'POST':
				if ($content=UTILITY::propertySet($this->_payload, LANG::PROPERTY('documentation.create_identifier'))) $content .= ' ' . date('Y-m-d H:i');
				if ($content){
					$downloadfiles = [];
					$downloadfiles[LANG::GET('documentation.create_identifier')] = [
						'href' => $this->identifierPDF($content)
					];
					$this->response([
						'log' => LANG::GET('documentation.create_identifier_proceed'),
						'links' => $downloadfiles
					]);
	
				}
				else $this->response(['status' => [
					'msg' => LANG::GET('documentation.create_identifier_error')
				]]);
			...
			case 'GET':
				...
				[
					'form' => [
						'data-usecase' => 'documentation',
						'action' => "javascript:api.documentation('post', 'identifier')"],
					'content'=>[
						[
							[
								'type' => 'text',
								'description' => LANG::GET('documentation.create_identifier_info')
							], [
								'type' => 'scanner',
								'hint' => LANG::GET('documentation.create_identifier_hint'),
								'attributes' => [
									'name' => LANG::GET('documentation.create_identifier'),
									'maxlength' => 128
								]
							]
						]
	...
	private function identifierPDF($content){
		...
		$pdf->SetTitle(LANG::GET('documentation.create_identifier'));
