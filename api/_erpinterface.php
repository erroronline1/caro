...
class ODEVAVIVA extends _ERPINTERFACE {
	...
	public function orderdata($from = null, $until = 'now'){
		...
		article.BESTELL_TEXT AS ARTIKELBEZEICHNUNG
...
//				'identifier' => string, part of order text previously pasted during order process generated by caro app in the scheme `  |sz9623` two space-pipe-base 36 unixtime according to UTILITY::identifier(), format accordingly
				...
				'ordered' => $row['BESTELL_DATUM'] ? substr($row['BESTELL_DATUM'], 0, -4) : null,
				'partially_received' => $row['WE_DATUM'] && $row['WE_MENGE'] != $row['BESTELL_MENGE'] ? substr($row['WE_DATUM'], 0, -4) : null,
				'received' => $row['WE_DATUM'] && $row['WE_MENGE'] == $row['BESTELL_MENGE'] ? substr($row['WE_DATUM'], 0, -4) : null,
			];
		}
		return $response;
	}
